...
{-| Create a regular checkbox:
    ...
    - `onChange` is the handler called when the state changes.
...
checkbox config =
    let checked =
        ...
        msg =
            config.onChange (inverse config.selected)
    in
    viewContainer config.identifier
        [ Html.checkbox config.identifier checked <|
            [ Events.onCheck (\_ -> msg)
            , Attributes.id config.identifier
            ]
        , viewLabelFrame config.identifier
            [ cursor pointer ]
            [ Widget.disabled False
            , Widget.checked checked
            , Attributes.tabindex 0
            , onKeyUp msg
            , classes config.selected
            ]
            [ viewIcon (checkmark config.selected)
            , viewLabel { text = config.label, visible = config.anonymous }
            ]
        ]
...
locked config =
    viewContainer config.identifier
        [ Html.checkbox config.identifier Nothing
            [ Attributes.id config.identifier
            , Attributes.disabled True
            ]
        , viewLabelFrame config.identifier
            ...
            [ Widget.disabled True
            , Widget.checked (Just False)
            , onKeyUp config.onClick
            , classes NotSelected
            ]
...
{-| The state of a checkbox:
    ...
    - `Selected` is checked (rendered with a checkmark)
    - `NotSelected` is not checked (rendered blank)
    - `PartiallySelected` is indeterminate (rendered dash)
...
`binary` lets you easily convert that into an `Selected` value
for use with `Nri.Ui.Checkbox`.

-}
binary : Bool -> Selected
binary isSelected =
    case isSelected of
        True ->
            Selected
...
viewContainer : String -> List (Html.Html msg) -> Html.Html msg
viewContainer identifier =
    Html.Styled.span
        [ css
            [ display block
            , height inherit
...
viewLabelFrame : String -> List Css.Style -> List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
viewLabelFrame identifier styles attrs =
    Html.Styled.label <|
        [ Attributes.for identifier
        , Aria.controls identifier
        , Attributes.tabindex 0
        , css (positioning :: textStyle :: styles)
        ] ++ attrs
...
viewLabel : { text : String, visible : Bool } -> Html.Html msg
viewLabel label =
    if label.visible then
        Html.span [] [ Html.text label.text ]
    else
...
onKeyUp : msg -> Html.Attribute msg
onKeyUp msg =
    HtmlExtra.onKeyUp { defaultOptions | preventDefault = True } <| \keyCode ->
...
selectedToMaybe : Selected -> Maybe Bool
...
classes : Selected -> Html.Styled.Attribute msg
classes selected =
    let classList =
            List.map namespace >> Attributes.classList

        namespace class =
            ( "checkbox-V6__" ++ class, True )
    in
    case selected of
        Selected ->
            classList [ "Label", "Checked" ]

        NotSelected ->
            classList [ "Label", "Unchecked" ]

        PartiallySelected ->
            classList [ "Label", "Indeterminate" ]
