module Nri.Ui.Table.V9 exposing
    ( Column, SortDirection(..), custom, string, rowHeader, placeholderColumn
    , Attribute, css, AlternatingRowColors(..), alternatingRowColors, backgroundChangeOnRowHover
...
alternatingRowColors alternatingColors =
    Attribute <|
        \config ->
            { config | alternatingRowColors = alternatingColors }
...
viewLoadingRow : List (Column data msg) -> AlternatingRowColors -> Int -> Html msg
viewLoadingRow columns alternatingColors index =
    tr
        [ Attributes.css
            (rowStyles
...
rowStyles rowConfig =
    ...
    , fontSize (px 14)
    , color gray20
    , case rowConfig.alternatingColors of
        Odd ->
