module Nri.Ui.ClickableText.V5 exposing
    ( button
    , link
    , Attribute
...
-}
hideIconFor : (List Style -> MediaQuery property) -> Attribute msg
        ...
        (\config ->
            { config
                | customStyles =
                    List.concat
                        [ config.customStyles
                        , MediaQuery.toStyles config.customResponsiveStyles
...
button label_ attributes =
    ...
    Nri.Ui.styled Html.button
        ...
        (List.concat
            [ clickableTextSharedStyles config.disabled
