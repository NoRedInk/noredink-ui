module Nri.Ui.SelectElement.V2 exposing
    ( view
    , Attribute(..)
...
optionBaseStyles =
    ...
    , borderRadius (px 4)
    , margin2 zero (px 8)
    , Css.property "--sel-opt-bg" (ColorsExtra.toCssString (Css.rgba 0 0 0 0))
    , Css.property "--sel-opt-fg" (ColorsExtra.toCssString Colors.navy)
    , Css.property "--sel-opt-bg-hover" (ColorsExtra.toCssString Colors.frost)
    , Css.property "--sel-opt-fg-hover" (ColorsExtra.toCssString Colors.navy)
    , Css.property "--sel-opt-bg-selected" (ColorsExtra.toCssString Colors.highlightBlueLight)
    , Css.property "--sel-opt-fg-selected" (ColorsExtra.toCssString Colors.navy)
    , Css.property "--sel-opt-bg-active" (ColorsExtra.toCssString Colors.cornflowerLight)
    , Css.property "--sel-opt-fg-active" (ColorsExtra.toCssString Colors.azureDark)
    , Css.property "background-color" "var(--sel-opt-bg-which, var(--sel-opt-bg))"
    , Css.property "color" "var(--sel-opt-fg-which, var(--sel-opt-fg))"
    , Css.pseudoClass "hover"
        [ Css.property "background-color" "var(--sel-opt-bg-hover)"
        , Css.property "color" "var(--sel-opt-fg-hover)"
        ]
    ]
...
renderSingleOption config popoverIdForOption valToString option =
    let
        ...
        isSelected =
            case config.selectedValue of
                Just val ->
                    valToString val == valToString option.value
        ...
        selectedStateStyles =
            if isSelected then
                [ Css.property "--sel-opt-bg-which" "var(--sel-opt-bg-selected)"
                , Css.property "--sel-opt-fg-which" "var(--sel-opt-fg-selected)"
    ...
    div
        ([ attribute "role" "option"
         ...
         , tabindex -1
         , Attributes.css (optionBaseStyles ++ optionSpecificStyles ++ selectedStateStyles)
         ]
            ++ (if isOptionDisabled then
                            ...
                            |> Decode.andThen
                                (\strVal ->
                                    case config.stringToValue strVal of
                                        ...
                                        Nothing ->
                                            Decode.fail ("SelectElement.V2: Could not convert string '" ++ strVal ++ "' back to value type.")
                                )
                        )
                    ]
                        ...
                        :: (case config.label of
                                Just lblString ->
                                    [ InputLabelInternal.view
                                        { for = config.triggerId
                                        , label = lblString
                                        , theme = InputStyles.Standard
                                        }
                                        { error = labelErrorState
                                        , noMarginTop = config.noMargin
